...
void createBinarySearchTree(BitNode *T,DataType keys[],int count)
{
    //创建根节点
    *T = (BitNode)malloc(sizeof(BitTree));
    if (!(*T)) {
    ...
    for (int i = 1; i < count; i++) {
        BitNode node = (BitNode)malloc(sizeof(BitTree));
        if (!node) {
            printf("malloc node failed");
            return;
        }
        ...
        while (tmp != NULL) {
            ...
            }else if(node->data < tmp->data){   //去找左节点
                if (tmp->lchild) {
                    tmp = tmp->lchild;
                }else{
                    tmp->lchild = node;
